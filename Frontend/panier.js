let enregistementDansLocalStorage = JSON.parse(
...
const afficherProduitPanier = () => {
  if (enregistementDansLocalStorage === null) {
    ...
    for (let i = 0; i < enregistementDansLocalStorage.length; i++) {
      ...
      divInfo.className =
        "border col-lg-4 rounded-0 mb-5 px-2 align-self-stretch";
      ...
      divTitreInfo.innerHTML =
        "Titre : " + enregistementDansLocalStorage[i].nomArticle;
      ...
      divReference.innerHTML =
        "Référence : " + enregistementDansLocalStorage[i].id;
      divInfo.appendChild(divReference);

      let divChoixLentille = document.createElement("p");
      divChoixLentille.innerHTML =
        "Choix lentille : " + enregistementDansLocalStorage[i].ChoixOption;
      ...
      let prixTotalDuProduit =
        enregistementDansLocalStorage[i].prix *
        enregistementDansLocalStorage[i].quantite;
      let prixCentimes = new Intl.NumberFormat("fr-FR", {
        style: "currency",
        currency: "EUR",
      }).format(prixTotalDuProduit / 100);
...
form.addEventListener("submit", (event) => {
  if (RegexLettre.test(firstName.value) == false) {
    ...
    indication.textContent =
      "Le champs est vide ou n'accepte pas les caractères spéciaux, veuillez ressaisir à nouveau vos informations.";
    ...
    event.preventDefault();
    indication.textContent =
      "Le champs est vide, veuillez saisir votre adresss mail.";
    indication.style.display = "block";

    // Validation du panier
  } else if (
    enregistementDansLocalStorage < 1 ||
    enregistementDansLocalStorage == null
  ) {
    event.preventDefault();
      ...
      fetch("http://localhost:3000/api/cameras/order", options)
        ...
        .then((response) => {
          ...
          localStorage.setItem(
            "Prix Total",
            JSON.stringify(EnregistrementPrixTotal)
          );
