//////// RECUPERATION ID DE L'URL(*plan test)
...
// connexion avec Fetch et ajout infos du produit dans le HTML(*plan test)
    ...
    .then((response) => {
      ...
      idData.then((arrayId) => {
        ...
        choixLentilles.addEventListener("change", () => {
          selectOption =
            choixLentilles.options[choixLentilles.selectedIndex].text;
        ...
        btnEnvoiPanier.addEventListener("click", (event) => {
          ...
          const confirmationPanier = () => {
            if (selectOption) {
              ...
              spanConfirmationOption.innerHTML =
          ...
          const ajoutProduitLocalStorage = () => {
            ...
            if (enregistementDansLocalStorage === null) {
              ...
              localStorage.setItem(
                "Articles",
                JSON.stringify(enregistementDansLocalStorage)
              );
              calculNombreArticle();
              confirmationPanier();
            } else {
              ...
              for (let i = 0; i < enregistementDansLocalStorage.length; i++) {
                ...
                if (
                  enregistementDansLocalStorage[i].id ===
                    ajoutProduitPanier.id &&
                  enregistementDansLocalStorage[i].ChoixOption ===
                    ajoutProduitPanier.ChoixOption
                ) {
                  produitIndex = i;
                }
              }
              ...
              if (produitIndex !== false) {
                enregistementDansLocalStorage[produitIndex].quantite =
                  Number(enregistementDansLocalStorage[produitIndex].quantite) +
                  Number(ajoutProduitPanier.quantite);
              } else {
                enregistementDansLocalStorage.push(ajoutProduitPanier);
              }
              localStorage.setItem(
